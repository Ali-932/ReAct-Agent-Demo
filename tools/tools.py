...
def convert_to_tool_format(func, description):
    ...
    logger.debug(f'inspect.signature(func) is {inspect.signature(func)}')
    logger.debug(f'inspect.signature(func).parameters is {inspect.signature(func).parameters}')
    ...
    logger.debug(f'param_description is {param_descriptions}')
    for param, details in parameters.items():
        ...
        data_type = type_mapping[param_type]
        # Add to properties
        properties[param] = {
            "type": data_type,
